  ...
  return (
    <div className="glass rounded-2xl p-6 overflow-hidden relative">
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Hourly */}
        <div>
          <p className="text-[11px] text-zinc-500 mb-3 font-medium">שעות היממה</p>
        ...
        {/* Weekday */}
        <div>
          <p className="text-[11px] text-zinc-500 mb-3 font-medium">ימי השבוע</p>
          <div className="space-y-[3px]">
            {weekday.map((w) => {
              const intensity = w.count / maxWeekday;
              return (
                <div key={w.day} className="flex items-center gap-2">
                  <span className="text-[10px] text-zinc-600 w-10 text-start">{w.day}</span>
                  ...
                  <span className="text-[10px] text-zinc-600 tabular-nums w-8 text-start">
                    {w.count}
                  </span>
                </div>
              );
