        ...
        {events.length > 0 ? (
          <div className="space-y-0.5">
            {events.map((event) => (
              ...
              >
                <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 bg-white/[0.04] border border-white/[0.06] text-zinc-300 transition-transform duration-200 group-hover:scale-105">
                  <Zap className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <EventBadge eventName={event.eventName} />
                  <p className="text-[12px] text-zinc-300 truncate mt-0.5">
                    {event.project.name}
                  </p>
                </div>
                <span className="text-[10px] text-zinc-300 flex-shrink-0 mt-1 font-medium">
