    ...
    <header className="h-[64px] bg-[#06060a]/50 backdrop-blur-md border-b border-white/[0.05] flex items-center justify-between px-8 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <Search className="absolute start-3.5 top-1/2 -translate-y-1/2 w-[15px] h-[15px] text-zinc-600" />
          <input
            type="text"
            placeholder="חיפוש..."
            className="w-64 ps-10 pe-4 py-2 bg-white/[0.03] border border-white/[0.06] rounded-lg text-[13px] text-zinc-300 placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-white/[0.1] focus:border-white/[0.1] transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button className="flex items-center gap-2.5 p-1.5 pe-3 rounded-lg hover:bg-white/[0.04] transition-colors duration-200">
          ...
          <div className="hidden md:block text-start">
            ...
            <p className="text-[10px] text-zinc-600">מנהל מערכת</p>
          </div>
          <ChevronDown className="w-3.5 h-3.5 text-zinc-600 hidden md:block" />
        </button>
      </div>
    </header>
  );
